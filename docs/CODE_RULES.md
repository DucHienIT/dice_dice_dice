...
```
Assets/Scripts/
├── Core/          # GameManager, state machine (Ready/Event/Battle/Choice), vòng đời run
├── Events/        # Event roll: weights, streak guard, fortune/spring/trap/treasure
├── Combat/        # Auto-battle theo nhịp, damage/crit, enrage
...
- **Data-driven bằng ScriptableObject:** mọi chỉ số cân bằng (event weights, `BEAT_MS`, chỉ số base của player, công thức scaling quái, crit, enrage, nội dung upgrade/sidekick, palette planet...) nằm trong file `.asset` dưới `Assets/Data/` (class định nghĩa ở `Assets/Scripts/Data/`) — KHÔNG hardcode số cân bằng trong logic. Designer phải chỉnh được số mà không sửa code. Spec mục "Hướng tuning" liệt kê đúng các nút vặn này.
- **Tách logic khỏi presentation:** logic gameplay (event roll, nhịp đánh, damage, XP/level) không phụ thuộc vào UI/VFX. UI lắng nghe qua C# `event`/`Action`; logic không gọi thẳng vào UI.
- **State machine trung tâm:** trạng thái run (Ready/Event/Battle/Choice/Dead...) quản lý ở một nơi (GameManager hoặc tương đương). Mọi hành vi phụ thuộc trạng thái (nút ENGAGE bấm được không, save được ghi lúc nào, upgrade panel mở/đóng) hỏi state này — không tự track state riêng lẻ ở từng component.
...
- **Object pooling bắt buộc** cho floater (số damage/heal/XP bay lên), hiệu ứng, quái/orb sidekick nếu spawn lặp lại — không `Instantiate`/`Destroy` liên tục trong gameplay.
...
- `Update` chỉ ở nơi thật sự cần chạy mỗi frame; timer đơn giản (beat timer, animation lunge) dùng cộng dồn `Time.deltaTime`, không tạo coroutine tràn lan.
...
Game chạy mobile; trong battle mỗi nhịp sinh floater + hiệu ứng liên tục. Mọi code trong gameplay loop phải viết theo tư duy "frame budget".

### 5.1. Zero allocation trong hot path

Hot path = mọi code chạy mỗi frame hoặc nhiều lần mỗi giây trong battle (`Update` của manager, beat tick, animation, floater). Trong hot path:
...
- Physics query (nếu có) dùng bản không cấp phát: overload nhận buffer/`List` cấp sẵn (Unity 6 khuyến nghị overload nhận `ContactFilter2D` + `List<T>`; các API `*NonAlloc` cũ một phần đã obsolete).
...
- Text cập nhật thường xuyên (HP, XP, damage): chỉ set khi giá trị đổi, không set mỗi frame; số format dùng cache chuỗi cho giá trị nhỏ lặp lại nếu profiler chỉ ra vấn đề.

### 5.2. Cấu trúc update hiệu quả

- **Update manager thay vì trăm `Update()`:** floater, hiệu ứng, beat timer... không tự có `Update()` riêng. Mỗi hệ thống có một manager giữ danh sách active object và tick chúng trong một `Update()` duy nhất (ví dụ `FloaterManager.Update()` lặp qua list floater). Nhiều `MonoBehaviour.Update()` riêng lẻ có overhead interop đáng kể.
...
- Pool cho: floater (damage/heal/XP), VFX, audio one-shot, và mọi object spawn lặp lại trong battle.
- **Prewarm** pool trong loading/trước battle — không để `Instantiate` xảy ra giữa trận gây spike.
...
- **Chia Canvas theo tần suất thay đổi:** một element thay đổi làm rebuild cả Canvas chứa nó. Tách riêng: Canvas tĩnh (khung, nền), Canvas HUD thay đổi thường xuyên (HP, XP bar, round/planet), Canvas popup/hiệu ứng. Không dồn tất cả vào một Canvas.
- Tắt `Raycast Target` trên mọi Image/Text không cần nhận input (đa số là không cần).
- Không dùng Layout Group (`HorizontalLayoutGroup`...) + `ContentSizeFitter` cho UI thay đổi trong gameplay — chúng rebuild đắt; layout động thì set vị trí bằng code. Layout Group chỉ chấp nhận được cho UI tĩnh dựng một lần (panel 3 thẻ upgrade, hàng chip sidekick).
...
- Background theo planet render/dựng **một lần khi warp** rồi giữ nguyên (spec quy định background cache theo planet) — không sinh lại chi tiết nền mỗi frame.
...
- Material dùng chung (shared) cho object cùng loại để batch được; **không** `renderer.material` (tạo instance material — phá batching và leak) — đổi màu per-object (màu quái procedural, palette planet) thì dùng `MaterialPropertyBlock` hoặc màu vertex/sprite color.
...
- Event đặt tên theo chuyện đã xảy ra (`OnBattleEnded`, `OnEventRolled`, `OnLevelUp`), subscriber tự lo việc của mình — publisher không biết ai nghe.
...
Áp dụng SOLID theo hướng thực dụng cho Unity — mục tiêu là code dễ mở rộng nội dung game (thêm event, upgrade, sidekick, planet mới) mà không sửa code cũ.
...
- **Không God object:** GameManager chỉ điều phối state machine và vòng đời run — không chứa logic combat, event roll, progression. Mỗi hệ thống trong mục 1 tự quản logic của mình.

### 7.2. SOLID áp dụng cụ thể

- **S — Single Responsibility:** một class, một lý do để thay đổi. Ví dụ combat tách thành: logic nhịp/damage, logic enrage, và view (animation lunge, floater) — sửa hiệu ứng đánh không đụng vào công thức damage.
- **O — Open/Closed:** thêm nội dung mới bằng cách **thêm** asset/class, không **sửa** code cũ. Cấm switch-case/if-else theo loại event, loại upgrade hay loại sidekick rải rác trong code (`switch (eventType)` lặp khắp nơi) — hành vi riêng của từng loại nằm trong class/ScriptableObject của loại đó (polymorphism hoặc data-driven). Thêm upgrade thứ 9 hay sidekick thứ 5 không được phép sửa file nào ngoài file của chính nó + data.
- **L — Liskov Substitution:** class con phải dùng được ở mọi nơi class cha được dùng. Không override method thành no-op hay throw để "vô hiệu hóa" hành vi thừa kế — nếu phải làm vậy nghĩa là kế thừa sai chỗ (ví dụ: sidekick là passive, không kế thừa từ class chiến đấu rồi vô hiệu hóa attack; tách hẳn nhánh passive và combatant).
- **I — Interface Segregation:** interface nhỏ theo vai trò: `IDamageable`, `ITickable`, `IEventHandler`... — không một interface khổng lồ bắt mọi loại implement cả những gì nó không có. Hệ thống chỉ phụ thuộc đúng interface nó cần (combat chỉ biết `IDamageable`, không biết đó là quái thường, elite hay boss).
...
- Logic gameplay phải tôn trọng tuyệt đối luật và công thức trong `docs/COSMIC_CRITTER_QUEST_Game_Design_Spec.md` — code không được "sáng tạo" thêm cơ chế; cân bằng chỉnh theo mục "Hướng tuning" của spec.
