...
- **UI uses the Layer Lab GUI Pro-FantasyHero skin** (frames/buttons/sliders/popups/icons + GermaniaOne TMP font), authored into the scene by the installer; state-swapped sprites live in `Assets/Data/UiSkin.asset`. The Dice icon comes from the GUI Pro-CasualGame pack (same vendor). **GermaniaOne/AfacadFlux SDF fonts have NO Vietnamese glyphs** — use them only for numbers/ASCII labels; Vietnamese text stays on the TMP default font. Layer Lab panels are light parchment, so UI body text defaults to dark brown.
