...
- **HARD RULE — single font:** every TMP text in the game uses exactly one font asset: `GermaniaOne-Regular SDF` (Layer Lab). It is assigned centrally in the installer's `CreateTmp`; never assign another font or a per-label variant. Its static atlas is ASCII-only, which is why game strings must be ASCII.
